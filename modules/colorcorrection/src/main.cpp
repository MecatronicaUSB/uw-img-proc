...
int main(int argc, char *argv[])
{
	...
	*/
	...
	{
		std::cout << endl
				  << "C++ implementation of color correction module using Gray World Assumption" << endl;
		std::cout << endl
				  << "Arguments are:" << endl;
		...
		std::cout << endl
				  << "Complete options of methods are:" << endl;
		...
		std::cout << "\tThis will open 'input.jpg' correct the color using GWA-Lab and save it in 'output.jpg'" << endl
				  << endl;
		return 0;
	}
	...
	if (src.empty())
	{
		std::cout << "Error occured when loading the image" << endl
				  << endl;
		return -1;
	}
	...
	if (CUDA)
	{
		...
		case 'L': // Lab
			std::cout << endl
					  << "Applying color correction using GWA-Lab" << endl;
			dstGPU = GWA_Lab_GPU(srcGPU);
			break;

		case 'C': // CIELAB
			std::cout << endl
					  << "Applying color correction using GWA-CIELAB" << endl;
			dstGPU = GWA_CIELAB_GPU(srcGPU);
			break;

		case 'R': // RGB
			std::cout << endl
					  << "Applying color correction using GWA-RGB" << endl;
	...
	if (Time)
	{
		...
		std::cout << endl
				  << "Execution Time" << implementation << ": " << t << " ms " << endl;

		// Name for the output csv file where the time will be saved
		std::size_t pos;
		if (OutputFile.find(92) != std::string::npos)
			pos = OutputFile.find_last_of(92); // If string contains '\' search for the last one
		else
			pos = OutputFile.find_last_of('/'); // If does not contain '\' search for the last '/'
		...
		file << endl
			 << OutputFile << ";" << src.rows << ";" << src.cols << ";" << t;
	}

	std::cout << endl
			  << "Saving processed image" << endl;
	imwrite(OutputFile, dst);

	if (Show)
	{
		std::cout << endl
				  << "Showing image comparison" << endl;
		cv::Mat comparison;
