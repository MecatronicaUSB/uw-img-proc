...
cv::Mat UCM(cv::Mat src, float percent) {
	...
	for (int i = 0; i < 3; i++) threshold(sorted_chan[i], sorted_chan[i], 255, 255, 2);	// Limit the max value to 255
	...
	cvtColor(result, HSV, COLOR_BGR2HSV);												// Conversion to HSV color space
	split(HSV, hsv);
...
cv::cuda::GpuMat UCM_GPU(cv::cuda::GpuMat srcGPU, float percent) {
	...
	for (int i = 0; i < 3; i++) result_chan[sorted.at<int>(0, i)] = histStretchGPU(sorted_chan[i], percent, i);
	cv::cuda::merge(result_chan, 3, result);

	cv::cuda::GpuMat HSV, hsv[3], dst;
	cv::cuda::cvtColor(result, HSV, COLOR_BGR2HSV);										// Conversion to HSV color space
	cv::cuda::split(HSV, hsv);
	for (int i = 1; i < 3; i++) hsv[i] = histStretchGPU(hsv[i], percent, 1);			// Histogram stretching of channels S and V
	cv::cuda::merge(hsv, 3, HSV);
